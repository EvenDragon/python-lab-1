...
def validate_convert(value: float, unit1: str, unit2: str) -> bool:
    ...
    Return True if no errors detected, in other cases return False
    """
    if unit1 in LENGTH_UNITS and unit2 in LENGTH_UNITS or unit1 in MASS_UNITS and unit2 in MASS_UNITS:
        if value < 0:
        ...
        if (
            (unit1 == TempUnits.celsius.value and value < -CELSIUS_FAHRENHEIT_ADD)
            or (unit1 == TempUnits.kelvin.value and value < 0)
            or (
                unit1 == TempUnits.fahrenheit.value and value < FAHRENHEIT_ABSOLUTE_ZERO
            )
        ):
            errors.throw_wrong_value()
            return False
